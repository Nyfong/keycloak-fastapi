...
def validate_env_vars():
    ...
    if not os.getenv("KEYCLOAK_URL").endswith(f"/realms/{os.getenv('KEYCLOAK_REALM_NAME')}"):
        raise ValueError("KEYCLOAK_URL must end with /realms/{REALM_NAME}")
    if not os.getenv("KEYCLOAK_ADMIN_URL").endswith("/admin"):
        raise ValueError("KEYCLOAK_ADMIN_URL must end with /admin")
...
async def get_admin_token() -> str:
    async with httpx.AsyncClient() as client:
        ...
        print("Admin token request payload:", {
            "grant_type": "password",
            "client_id": CLIENT_ID,
        ...
        if response.status_code != 200:
            ...
            raise HTTPException(status_code=response.status_code, detail=f"Admin authentication failed: {error_detail}")
...
async def login(login_request: LoginRequest) -> dict:
    try:
        async with httpx.AsyncClient() as client:
            ...
            print("Login request URL:", f"{KEYCLOAK_URL}/protocol/openid-connect/token")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            users_response = await client.get(
                f"{KEYCLOAK_ADMIN_URL}/realms/{REALM_NAME}/users?username={user_data.username}&exact=true",
                headers={"Authorization": f"Bearer {admin_token}"},
            )
            print("User retrieval URL:", f"{KEYCLOAK_ADMIN_URL}/realms/{REALM_NAME}/users?username={user_data.username}&exact=true")
            ...
            if password_response.status_code != 204:
                raise HTTPException(status_code=500, detail=f"Failed to set password")
...
async def get_social_login_url() -> dict:
    ...
    print("Social login URL:", auth_url)
...
async def social_login_callback(code: str) -> dict:
    try:
        async with httpx.AsyncClient() as client:
            ...
            print("Social login token URL:", f"{KEYCLOAK_URL}/protocol/openid-connect/token")
            print("Social login token payload:", {
                "grant_type": "authorization_code",
                "client_id": CLIENT_ID,
                "client_secret": "[REDACTED]",
                "code": code,
            ...
            print("Social login response:", response.status_code, response.text)
            if response.status_code != 200:
